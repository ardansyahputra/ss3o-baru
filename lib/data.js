...
// getStaffProgressMap() — ringkasan progress & jobdesk hari ini PER staff,
// dipakai halaman admin yang butuh info ini di luar Monitoring (mis. halaman
// Kelola Bukti Upload) supaya panel "Proof Inspection" tidak lagi selalu
// menampilkan Progress 0% / Jobdesk 0 walau staff-nya sudah punya jobdesk &
// upload. Return-nya di-index by userId biar gampang di-lookup.
async function getStaffProgressMap() {
  const allUsers = await db.all("users");
  const users = allUsers.filter((item) => item.isActive !== false);
  const departments = await departmentMap();
  const reports = await latestReports();
...
module.exports = { dayKey, getDashboardData, getMonitoringData, getStaffJobdesks, getStaffProgressMap, getUserJobdesks, reviewStatusLabel, statusClass, statusLabel, uploadRowsFor };
